...
TEST_RETURN = {
    ...
    "data_source": "https://www.google.com/search?lr=lang_en&q=weather+in+dublin",
}
...
def mocked_request(monkeypatch):
    ...
    monkeypatch.setattr(
        "weather_api_functions.requests",
        mocked_request,
    )
    return mocked_request


# python -m pytest test_weather_api.py::test_sunny_patched_w_fixture
def test_sunny_patched_w_fixture(mocked_request):
    """
    Use mocked_request to patch the requests library used in weather_api_functions

    Then later on, put a mock on the get method.
    """
    mock_return = mock.Mock()
    mock_return.json.return_value = TEST_RETURN
    mocked_request.get.return_value = mock_return
    sunny_result = weather_api_functions.sunny("Dublin")
    assert sunny_result is False
